class UnimplementedMethodExtractor
  ...
  def getInfoDefaultCase(buildLog)
    ...
    while(count < interfaceFiles.size)
      ...
      fileOnePath = ""
      methodInterface = ""
      if (buildLog.match(/\[ERROR\] [0-9a-zA-Z\/\-]*\.java/).to_s.match(/[\-a-zA-Z0-9\/.]+\.java/)[0].to_s)
        classFile = classFiles[count].to_s.match(/[a-zA-Z]+\.java/)[0].to_s.gsub(".java", "")
        fileOnePath = classFiles[count].to_s.match(/[\-a-zA-Z0-9\/.]+\.java/)[0].to_s
      ...
      fileTwoPath = interfaceFiles[count].to_s.split(" ").last.gsub("\r", "").to_s
