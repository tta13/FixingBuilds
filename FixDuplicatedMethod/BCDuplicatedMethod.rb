...
class BCDuplicatedMethod
  ...
  def gumTreeDiffByBranch(result, left, right, base, pathProject, cloneProject)
    ...
    return verifyBuildConflict(baseLeft, leftResult, baseRight, rightResult, @conflictCauses)
  end
  ...
  def verifyBuildConflict(baseLeft, leftResult, baseRight, rightResult, filesConflicting)
    ...
    if (filesConflicting[0][2] != "method")
      puts "entrei"
      if(baseRight[0][filesConflicting[0][1]] != nil and baseRight[0][filesConflicting[0][1]].to_s.match(/Insert SimpleName: #{filesConflicting[0][2]}[0-9\(\)]* into [a-zA-Z\:\= 0-9\(\)]* on Method #{filesConflicting[0][3]}/))
        if (baseLeft[0][filesConflicting[0][1]] != nil and baseLeft[0][filesConflicting[0][1]].to_s.match(/Insert SimpleName: #{filesConflicting[0][2]}[0-9\(\)]* into [a-zA-Z\:\= 0-9\(\)]* on Method #{filesConflicting[0][3]}/))
          return true
        end
      end
      if(baseLeft[0][filesConflicting[0][1]] != nil and baseLeft[0][filesConflicting[0][1]].to_s.match(/Insert SimpleName: #{filesConflicting[0][2]}[0-9\(\)]* into [a-zA-Z\:\= 0-9\(\)]* on Method #{filesConflicting[0][3]}/))
      ...
      begin
        if(baseLeft[0][filesConflicting[0][1]] != nil and baseLeft[0][filesConflicting[0][1]].to_s.match(/Insert SimpleName: #{filesConflicting[0][3]}[0-9\(\)]* into [a-zA-Z]*[0-9\(\)]*/) and baseRight[0][filesConflicting[0][1]] != nil and baseRight[0][filesConflicting[0][1]].to_s.match(/Insert SimpleName: #{filesConflicting[0][3]}[0-9\(\)]* into [a-zA-Z]*[0-9\(\)]*/))
          return true
        end
      rescue
