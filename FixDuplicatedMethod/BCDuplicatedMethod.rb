...
class BCDuplicatedMethod
  ...
  def verifyBuildConflict(baseLeft, leftResult, baseRight, rightResult, filesConflicting)
    count = 0    #while(count < filesConflicting.size)
    #    begin
    ...
    if (filesConflicting[0][1] != "method")
      ...
      begin
        if(baseLeft[0][filesConflicting[0][0]] != nil and baseLeft[0][filesConflicting[0][0]].to_s.match(/Insert SimpleName: #{filesConflicting[0][2]}[0-9\(\)]* into [a-zA-Z]*[0-9\(\)]*/) and baseRight[0][filesConflicting[0][0]] != nil and baseRight[0][filesConflicting[0][0]].to_s.match(/Insert SimpleName: #{filesConflicting[0][2]}[0-9\(\)]* into [a-zA-Z]*[0-9\(\)]*/))
          return true
        end
      rescue
